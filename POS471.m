...
% Anzahl der Konturlinien regulieren % hierzu Vektor mit Wertebereich definieren
lo_lim=nanmin(nanmin(Zs'));
up_lim=nanmax(nanmax(Zs'));
res=0.1; s_scale=lo_lim:res:up_lim;
...
% Farbskala
h=colorbar('fontsize',12,'fontweight','b');
% Farbpalette an Zahl der Konturlinien anpassen % Farbpalette ist "jet"
cm=colormap(jet(length(s_scale)-1));
% Farbachse lässt sich so verschieben
caxis([min(s_scale) max(s_scale)]);
hold on
...
% Konturlinien beschriften hl=clabel(C,h);
set(hl,'FontSize',10,'FontWeight','demi','color','k');
